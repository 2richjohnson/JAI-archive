...
# Step 1 — OCR: raw/ → ocr/
echo ""
echo "[1/4] OCR  (raw/ → ocr/)"
bash ~/jai-archive/01_ocr.sh

# Step 2 — Markdown conversion: ocr/ → markdown/
echo ""
echo "[2/4] Markdown conversion  (ocr/ → markdown/)"
python3 02_convert.py

# Step 3 — EAV extraction: markdown/ → tables/parquet/
echo ""
echo "[3/4] EAV extraction  (markdown/ → tables/parquet/)"
python3 05_extract_tables.py --workers 2

# Step 4 — Rebuild DuckDB
echo ""
echo "[4/4] Rebuilding DuckDB"
